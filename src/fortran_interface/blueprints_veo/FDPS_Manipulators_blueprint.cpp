...
namespace FDPS_Manipulators {
   ...
   class VeoFunctorEpEp : public VEO::FunctorEpEp<Tepi,Tepj> {
   public:
#if defined(VEO_USE_PROFILE)
      VeoFunctorEpEp(const char* pFuncPrepareEpjSymbol, const char* pFuncEpEpSymbol) : VEO::FunctorEpEp<Tepi,Tepj>(0, pFuncPrepareEpjSymbol, pFuncEpEpSymbol) {}
#else
      VeoFunctorEpEp(const char* pFuncPrepareEpjSymbol, const char* pFuncEpEpSymbol) : VEO::FunctorEpEp<Tepi,Tepj>(pFuncPrepareEpjSymbol, pFuncEpEpSymbol) {}
#endif
   };

   template<class Tepi, class Tepj, class Tspj>
   class VeoFunctorEpSp : public VEO::FunctorEpSp<Tepi,Tepj,Tspj> {
   public:
#if defined(VEO_USE_PROFILE)
      VeoFunctorEpSp(const char* pFuncPrepareSpjSymbol, const char* pFuncEpSpSymbol, VEO::FunctorEpEp<Tepi,Tepj>& functorEpEp) : VEO::FunctorEpSp<Tepi,Tepj,Tspj>(0, pFuncPrepareSpjSymbol, pFuncEpSpSymbol, functorEpEp) {}
#else
      VeoFunctorEpSp(const char* pFuncPrepareSpjSymbol, const char* pFuncEpSpSymbol, VEO::FunctorEpEp<Tepi,Tepj>& functorEpEp) : VEO::FunctorEpSp<Tepi,Tepj,Tspj>(pFuncPrepareSpjSymbol, pFuncEpSpSymbol, functorEpEp) {}
#endif
   };
#endif
   ...
   public:
      ...
      std::string EPJ;
      std::string mode;
      void *pVeoFunctorEpEp;
      void *pVeoFunctorEpSp;
   };
...
#if defined(_SX_VEOFFLOAD)
   void VEO_Initialize(char *veo_dll) {
      // Initialize VEOffloading Interface
      VEO::VeoInterface::initialize((const char *)argv_save[0],(const char *)veo_dll);
   }
#endif
   //--------------------------------
...
//#if defined(_SX_VEOFFLOAD)
   void create_tree(int *tree_num,
                    char *tree_info, const char* pFuncPrepareEpjSymbol, const char* pFuncEpEpSymbol,
                    const char* pFuncPrepareSpjSymbol, const char* pFuncEpSpSymbol) {
//#else
//   void create_tree(int *tree_num,
//                    char *tree_info) {
//#endif
   ...
   void delete_tree(const int tree_num) {
      for (std::vector<Tree_Data>::iterator it = tree_vector.begin(); it != tree_vector.end(); ++it) {
         if (it->id == tree_num) {
#if defined(_SX_VEOFFLOAD)
           Tree_Data* p_tree_data = &*it;
            //delete it->pVeoFunctorEpEp;
            //------------------------------------------------------
            // fdps-autogen:delete_tree;
            //------------------------------------------------------
#endif
   ...
   void dump_profile_tree(const int tree_num, const char* p_title) {
      void * tree_ptr;
      for (std::vector<Tree_Data>::iterator it = tree_vector.begin(); it != tree_vector.end(); ++it) {
         if (it->id == tree_num) {
                tree_ptr = it->ptr;
                ...
                Tree_Data* p_tree_data = &*it;
                //------------------------------------------------------
                // fdps-autogen:dump_profile_tree;
                //------------------------------------------------------
#endif
          }
      }
   ...
   void calc_force_all_and_write_back(const int tree_num, 
      ...
      for (std::vector<Tree_Data>::iterator it = tree_vector.begin(); it != tree_vector.end(); ++it) {
         if (it->id == tree_num) {
            tree_ptr = it->ptr;
#if defined(_SX_VEOFFLOAD)
            func_ep_ep_ptr = it->pVeoFunctorEpEp;
            func_ep_sp_ptr = it->pVeoFunctorEpSp;
#endif
            info += ",";
            info += it->info;
            is_exist = true;
...
#if defined(_SX_VEOFFLOAD)
            func_ep_ep_ptr = it->pVeoFunctorEpEp;
            func_ep_ep_ptr = it->pVeoFunctorEpSp;
#endif
            info = it->info;
            is_exist = true;
            break;
