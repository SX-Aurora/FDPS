...
module FDPS_module
   ...
   contains
      ...
      !-(Tree functions)
      procedure,private :: create_tree_s
      procedure,private :: create_tree_l
      generic:: create_tree => create_tree_s, &
                               create_tree_l
      procedure :: delete_tree
      procedure :: dump_profile_tree
      procedure :: init_tree
   ...
   !-(Tree functions)
   private :: create_tree_s
   private :: create_tree_l
   private :: delete_tree
   private :: dump_profile_tree
   private :: init_tree
   ...
   interface
      ...
      subroutine fdps_veo_initialize(veo_dll) bind(c)
         use, intrinsic :: iso_c_binding
         implicit none
         character(kind=c_char), dimension(*), intent(in) :: veo_dll
      end subroutine fdps_veo_initialize
      ...
      !  Tree 
      !----------------------
      subroutine fdps_create_tree(tree_num,tree_info, &
                                   prepare_proc_name, &
                                   calc_proc_name, &
                                   prepare_spj_proc_name, &
                                   calc_spj_proc_name) bind(c)
         use, intrinsic :: iso_c_binding
         implicit none
         integer(kind=c_int), intent(inout) :: tree_num
         character(kind=c_char), dimension(*), intent(in) :: tree_info
         character(kind=c_char), dimension(*), intent(in) :: prepare_proc_name
         character(kind=c_char), dimension(*), intent(in) :: calc_proc_name
         character(kind=c_char), dimension(*), intent(in) :: prepare_spj_proc_name
         character(kind=c_char), dimension(*), intent(in) :: calc_spj_proc_name
      ...
      subroutine fdps_dump_profile_tree(tree_num,p_title) bind(c)
         use, intrinsic :: iso_c_binding
         implicit none
         integer(kind=c_int), value, intent(in) :: tree_num
         character(kind=c_char), dimension(*), intent(in) :: p_title
      end subroutine fdps_dump_profile_tree
   ...
   !----------------------------------------------------------
   subroutine VEO_initialize(this,veo_dll)
      implicit none
      class(FDPS_controller) :: this
      character(len=*,kind=c_char), intent(IN) :: veo_dll

      call fdps_veo_initialize(veo_dll)

   end subroutine VEO_initialize 
   ...
   !##########################################################
   subroutine create_tree_s(this,tree_num,tree_info_in, &
                                     prepare_proc_name_in, &
                                     calc_proc_name_in)
      implicit none
      class(FDPS_controller) :: this
      integer(kind=c_int), intent(INOUT) :: tree_num
      character(len=*,kind=c_char), intent(IN) :: tree_info_in
      character(len=*,kind=c_char), intent(IN) :: prepare_proc_name_in
      character(len=*,kind=c_char), intent(IN) :: calc_proc_name_in
     !character(len=:,kind=c_char), allocatable :: tree_info
      ...
      character(len=len(prepare_proc_name_in)+1,kind=c_char) :: prepare_proc_name
      character(len=len(calc_proc_name_in)+1,kind=c_char) :: calc_proc_name

      tree_info = trim(tree_info_in) // c_null_char
      prepare_proc_name = trim(prepare_proc_name_in) // c_null_char
      calc_proc_name = trim(calc_proc_name_in) // c_null_char
      call fdps_create_tree(tree_num,tree_info,prepare_proc_name,calc_proc_name,c_null_char, c_null_char)

   end subroutine create_tree_s

   subroutine create_tree_l(this,tree_num,tree_info_in, &
                                prepare_proc_name_in, &
                                calc_proc_name_in, &
                                prepare_spj_proc_name_in, &
                                calc_spj_proc_name_in)
      implicit none
      class(FDPS_controller) :: this
      integer(kind=c_int), intent(INOUT) :: tree_num
      character(len=*,kind=c_char), intent(IN) :: tree_info_in
      character(len=*,kind=c_char), intent(IN) :: prepare_proc_name_in
      character(len=*,kind=c_char), intent(IN) :: calc_proc_name_in
      character(len=*,kind=c_char), intent(IN) :: prepare_spj_proc_name_in
      character(len=*,kind=c_char), intent(IN) :: calc_spj_proc_name_in
     !character(len=:,kind=c_char), allocatable :: tree_info
      ! See the comment in API create_psys for this comment-out.
      character(len=len(tree_info_in)+1,kind=c_char) :: tree_info
      character(len=len(prepare_proc_name_in)+1,kind=c_char) :: prepare_proc_name
      character(len=len(calc_proc_name_in)+1,kind=c_char) :: calc_proc_name
      character(len=len(prepare_spj_proc_name_in)+1,kind=c_char) :: prepare_spj_proc_name
      character(len=len(calc_spj_proc_name_in)+1,kind=c_char) :: calc_spj_proc_name

      tree_info = trim(tree_info_in) // c_null_char
      prepare_proc_name = trim(prepare_proc_name_in) // c_null_char
      calc_proc_name = trim(calc_proc_name_in) // c_null_char
      prepare_spj_proc_name = trim(prepare_spj_proc_name_in) // c_null_char
      calc_spj_proc_name = trim(calc_spj_proc_name_in) // c_null_char
      call fdps_create_tree(tree_num,tree_info,prepare_proc_name,calc_proc_name,prepare_spj_proc_name, calc_spj_proc_name)

   end subroutine create_tree_l
   ...
   !----------------------------------------------------------
   subroutine dump_profile_tree(this,tree_num,p_title)
      implicit none
      class(FDPS_controller) :: this
      integer(kind=c_int), intent(IN) :: tree_num
      character(len=*,kind=c_char), intent(IN) :: p_title

      call fdps_dump_profile_tree(tree_num,p_title)

   end subroutine dump_profile_tree
