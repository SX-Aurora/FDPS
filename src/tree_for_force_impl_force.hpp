...
#pragma omp parallel for
	...
	ftrace_region_begin("calc_force");
	...
	ftrace_region_begin("calc_force__core");
	...
	const auto len_peri = p_domain_info_->getLenRootDomain();
		...
		ftrace_region_begin("calc_force__core__walk_tree");
...
#pragma omp parallel for
...
#if defined(_SXDEBUG)
                time_profile_.calc_force__core__multiwalk_tree += GetWtime() - offset_calc_force__core__walk_tree;
#else
                time_profile_.calc_force__core__walk_tree += GetWtime() - offset_calc_force__core__walk_tree;
#endif
		ftrace_region_end("calc_force__core__walk_tree");
                if(!first_loop){
        ...
        ftrace_region_end("calc_force__core");
        ftrace_region_begin("calc_force__copy_original_order");
        ...
        ftrace_region_end("calc_force__copy_original_order");
...
#if defined(_SX_VEOFFLOAD)
	pfunc_ep_ep.transferEpiEpj(epi_sorted_.getPointer(), epi_sorted_.size(), epj_sorted_.getPointer(), epj_sorted_.size());
	time_profile_.calc_force__core__transfer += GetWtime() - time_offset;
#endif
...
#pragma omp parallel for schedule(dynamic, 4) reduction(+ : ni_tmp, nj_tmp, n_interaction_ep_ep_tmp)
...
#if defined(_SX_VEOFFLOAD)
                time_profile_.calc_force__core__walk_tree[ith] += GetWtime() - offset_walk_tree;
#else
		...
		ftrace_region_begin("calc_force__core__dispatch");
                calcForceOnly( pfunc_ep_ep, i, clear);
#if defined(_SX_VEOFFLOAD)
                time_profile_.calc_force__core__dispatch[ith] += GetWtime() - offset_dispatch;
#else
                time_profile_.calc_force__core__dispatch += GetWtime() - offset_dispatch;
#endif
		ftrace_region_end("calc_force__core__dispatch");
            }
    ...
    calcForceOnly(Tfunc_ep_ep pfunc_ep_ep,
        ...
        pfunc_ep_ep(
                ith,
                offset * sizeof(Tepi), n_epi,
                adr_epj_for_force_[ith].getPointer(), n_epj * sizeof(Tepj), n_epj,
                force_sorted_.getPointer(offset), n_epi * sizeof(Tforce)
                );
#else
        pfunc_ep_ep(
                ith,
                offset * sizeof(Tepi), n_epi,
                epj_for_force_[ith].getPointer(), n_epj * sizeof(Tepj), n_epj,
                force_sorted_.getPointer(offset), n_epi * sizeof(Tforce)
                );
#endif
        }
...
#if defined(_SXDEBUG)
        time_profile_.addEpEpDispatchInfo(ith, GetWtime() - time_offset, n_epi, n_epj);
#endif
	...
	ftrace_region_begin("calc_force");
        ...
        pfunc_ep_sp.transferSpj(spj_sorted_.getPointer(), spj_sorted_.size());
#endif
#endif
...
#if defined(_SXDEBUG)
                    offset_dispatch = GetWtime();
                    const F64 offset_dispatch_ep_ep = GetWtime();
#endif
                    pfunc_ep_ep(epi_sorted_.getPointer(offset),     n_epi,
                                epj_for_force_[ith].getPointer(),   n_epj,
                                force_sorted_.getPointer(offset));
#if defined(_SXDEBUG)
                    time_profile_.addEpEpDispatchInfo(ith, GetWtime() - offset_dispatch_ep_ep, n_epi, n_epj);
                    if (n_spj > 0) {

                    const F64 offset_dispatch_ep_sp = GetWtime();
#endif
                    pfunc_ep_sp(epi_sorted_.getPointer(offset),     n_epi,
                                spj_for_force_[ith].getPointer(),   n_spj,
                                force_sorted_.getPointer(offset));
#if defined(_SXDEBUG)
                    time_profile_.addEpSpDispatchInfo(ith, GetWtime() - offset_dispatch_ep_sp, n_epi, n_spj);
                    }
#endif
#if defined(_SXDEBUG)
                    time_profile_.calc_force__core__dispatch[ith] += GetWtime() - offset_dispatch;
...
//                time_profile_.calc_force__core__dispatch += GetWtime() - offset_dispatch;
    ...
    calcForceOnly(Tfunc_ep_ep pfunc_ep_ep,
        ...
        const S32 n_tail = offset + n_epi;
#if defined(_SXDEBUG)
        const F64 dispatch_ep_ep_offset = GetWtime();
#endif
#if defined(_SX_VEOFFLOAD)
        if(clear){
            for(S32 i=offset; i<n_tail; i++) force_sorted_[i].clear();
...
#endif
#if defined(_SXDEBUG)
        time_profile_.addEpEpDispatchInfo(ith, GetWtime() - dispatch_ep_ep_offset, n_epi, n_epj);
        if (n_spj > 0) {
        const F64 dispatch_ep_sp_offset = GetWtime();
#endif
#if defined(PREPARE_SPJ_IN_VE)
        pfunc_ep_sp(
                ith,
                offset * sizeof(Tepi), n_epi,
                adr_spj_for_force_[ith].getPointer(), n_spj * sizeof(Tspj), n_spj,
                force_sorted_.getPointer(offset), n_epi * sizeof(Tforce));
#else
        pfunc_ep_sp(
                ith,
                offset * sizeof(Tepi), n_epi,
                spj_for_force_[ith].getPointer(), n_spj * sizeof(Tspj), n_spj,
                force_sorted_.getPointer(offset), n_epi * sizeof(Tforce));
#endif

#if defined(_SXDEBUG)
        time_profile_.addEpSpDispatchInfo(ith, GetWtime() - dispatch_ep_sp_offset, n_epi, n_spj);
        }

#endif

#else // not defined _SX_VEOFFLOAD
...
#if defined(_SX_VEOFFLOAD)
#if defined(PREPARE_EPJ_IN_VE)
                ReallocatableArray<S32> adr_epj_for_force;
                for(S32 j=adr_epj_head; j<adr_epj_end; j++, n_cnt++){
                    const S32 adr_epj = interaction_list_.adr_ep_[j];
                    adr_epj_for_force.push_back(adr_epj);
                }
                pfunc_ep_ep(
                ith,
                adr_epi_head * sizeof(Tepi), n_epi,
                adr_epj_for_force.getPointer(), n_epj * sizeof(Tepj), n_epj,
                force_sorted_.getPointer(adr_epi_head), n_epi * sizeof(Tforce)
                );
#else
                for(S32 j=adr_epj_head; j<adr_epj_end; j++, n_cnt++){
                    const S32 adr_epj = interaction_list_.adr_ep_[j];
...
#if defined(_SXDEBUG)
                time_profile_.calc_force__core__dispatch[ith] += GetWtime() - offset_dispatch;
                time_profile_.addEpEpDispatchInfo(ith, GetWtime() - offset_dispatch, n_epi, n_epj);
#endif
            }
        }
        n_interaction_ep_ep_local_ += n_interaction_ep_ep_tmp;
...
#pragma omp parallel for schedule(dynamic, 4) reduction(+ : n_interaction_ep_ep_tmp, n_interaction_ep_sp_tmp)
            ...
            for(S32 i=0; i<n_ipg; i++){
                ...
                for(S32 j=adr_epj_head; j<adr_epj_end; j++){
                    const S32 adr_epj = interaction_list_.adr_ep_[j];
                    assert(adr_epj>=0 && adr_epj < epj_sorted_.size());
                    adr_epj_for_force.push_back(adr_epj);
                }
                ...
                ith,
                adr_epi_head * sizeof(Tepi), n_epi,
                adr_epj_for_force.getPointer(), adr_epj_for_force.size() * sizeof(Tepj), adr_epj_for_force.size(),
                force_sorted_.getPointer(adr_epi_head), n_epi * sizeof(Tforce));
                time_profile_.calc_force__core__dispatch[ith] += GetWtime() - offset_dispatch;
#else
                S32 n_ep_cnt = 0;
                ...
                offset_dispatch = GetWtime();
#if defined(_SXDEBUG)
                const F64 offset_ep_sp_dispatch = GetWtime();
#endif
#if defined(PREPARE_SPJ_IN_VE)
                pfunc_ep_sp(
                ith,
                adr_epi_head * sizeof(Tepi), n_epi,
                adr_spj_for_force.getPointer(), n_sp_cnt * sizeof(Tspj), n_sp_cnt,
                force_sorted_.getPointer(adr_epi_head), n_epi * sizeof(Tforce));
                ...
                ith,
                adr_epi_head * sizeof(Tepi), n_epi,
                spj_for_force_[ith].getPointer(), n_sp_cnt * sizeof(Tspj), n_sp_cnt,
                force_sorted_.getPointer(adr_epi_head), n_epi * sizeof(Tforce));
#endif
#if defined(_SXDEBUG)
                time_profile_.addEpSpDispatchInfo(ith, GetWtime() - offset_ep_sp_dispatch, n_epi, n_spj);
#endif
                time_profile_.calc_force__core__dispatch[ith] += GetWtime() - offset_dispatch;
                }
#else  // ! defined(_SX_VEOFFLOAD)
