...
int main(int argc, char *argv[]) {
    ...
    PS::TreeForForceLong<ForceGrav, FPGrav, FPGrav>::Monopole tree_grav;
    ...
    PS::F64 main_calc_force_time = 0.0;
    PS::F64 main_calc_force_offset;
#endif
...
#else
#if defined(_SXDEBUG)
	main_calc_force_offset = PS::GetWtime();
#endif
        tree_grav.calcForceAllAndWriteBack(CalcGravity<FPGrav>,
                                           CalcGravity<PS::SPJMonopole>,
                                           system_grav,
                                           dinfo);
#if defined(_SXDEBUG)
	main_calc_force_time += PS::GetWtime() - main_calc_force_offset;
#endif
#endif
    ...
    tree_grav.getTimeProfile().dump("TreeGrav");
    ...
    fprintf(stdout, "Rank=%d %s %s=%lf\n", PS::Comm::getRank(), "main", "main_calc_force_time", main_calc_force_time);
#endif
