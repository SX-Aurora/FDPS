...
class CalcDensity : public VEO::FunctorEpEp<EP,EP> {
   ...
   CalcDensity(const char* pFuncPrepareEpjSymbol, const char* pFuncEpEpSymbol) : VEO::FunctorEpEp<EP,EP>(1, pFuncPrepareEpjSymbol, pFuncEpEpSymbol) {
#else
   CalcDensity(const char* pFuncPrepareEpjSymbol, const char* pFuncEpEpSymbol) : VEO::FunctorEpEp<EP,EP>(pFuncPrepareEpjSymbol, pFuncEpEpSymbol) {
#endif
...
class CalcHydroForce : public VEO::FunctorEpEp<EP,EP> {
   public:
#if defined(VEO_USE_PROFILE)
   CalcHydroForce(const char* pFuncPrepareEpjSymbol, const char* pFuncEpEpSymbol) : VEO::FunctorEpEp<EP,EP>(2, pFuncPrepareEpjSymbol, pFuncEpEpSymbol) {
#else
   CalcHydroForce(const char* pFuncPrepareEpjSymbol, const char* pFuncEpEpSymbol) : VEO::FunctorEpEp<EP,EP>(pFuncPrepareEpjSymbol, pFuncEpEpSymbol) {
#endif
   }
};
...
void SetupIC(PS::ParticleSystem<FP>& sph_system, PS::F64 *end_time, boundary *box){
   ...
   //const PS::F64 dx = 1.0 / 128.0 / 2.9995;
   box->x = 1.0;
...
int main(int argc, char* argv[]){
   ...
   //sph_system.getTimeProfile().dump("Sph_System");
#endif
