...
module user_defined_types
   ...
   !-------------------------------------------------------------------
   subroutine subgradW(gradW,dr,h)
      implicit none
   ...
   subroutine calc_density(ep_i,n_ip,ep_j,n_jp,f) bind(c)
      ...
      !print '("calc_density n_ip=",i0," n_jp=", i0)', n_ip, n_jp
      ...
      !print '("f(", i0, ")%dens=",f10.8)',  n_ip, f(n_ip)%dens 

   end subroutine calc_density

   !**** prepare epj for force
   subroutine calc_density_prepare_epj_for_force(sorted_epj, adr_epj_for_force, n_jp, epj_for_force) bind(c) 
      integer(kind=c_int), intent(in), value :: n_jp
      type(essential_particle), dimension(n_jp), intent(in) :: sorted_epj
      integer(kind=c_int), dimension(n_jp), intent(in) :: adr_epj_for_force
      type(essential_particle), dimension(n_jp), intent(inout) :: epj_for_force
   ...
   end subroutine calc_density_prepare_epj_for_force

   subroutine calc_hydro_prepare_epj_for_force(sorted_epj, adr_epj_for_force, n_jp, epj_for_force) bind(c) 
      integer(kind=c_int), intent(in), value :: n_jp
      type(essential_particle), dimension(n_jp), intent(in) :: sorted_epj
      integer(kind=c_int), dimension(n_jp), intent(in) :: adr_epj_for_force
      type(essential_particle), dimension(n_jp), intent(inout) :: epj_for_force
   ...
   end subroutine calc_hydro_prepare_epj_for_force

   !**** Interaction function
   subroutine calc_hydro(ep_i,n_ip,ep_j,n_jp,f) bind(c)
      integer(kind=c_int), intent(in), value :: n_ip,n_jp
      type(essential_particle), dimension(n_ip), intent(in) :: ep_i
      type(essential_particle), dimension(n_jp), intent(in) :: ep_j
      ...
      type(fdps_f64vec) :: pos_i,pos_j,vel_i,vel_j, &
                           dr,dv,gradW_ij,gradW_smth_i,gradW_smth_j

      !print '("calc_hydro n_ip=",i0," n_jp=", i0)', n_ip, n_jp
	 ...
	 f(i)%eng_dot = 0.0
         ...
         do j=1,n_jp
            ...
            call subgradW(gradW_smth_i,dr,smth_i)
            call subgradW(gradW_smth_j,dr,smth_j)
            gradW_ij%x = 0.5d0 * (gradW_smth_i%x + gradW_smth_j%x)
            gradW_ij%y = 0.5d0 * (gradW_smth_i%y + gradW_smth_j%y)
            gradW_ij%z = 0.5d0 * (gradW_smth_i%z + gradW_smth_j%z)
