...
subroutine f_main()
   ...
   double precision :: offset_loop_duration, main_loop_duration
   double precision :: offset_file_output, file_output_time

   !* Initialize FDPS
   !call fdps_ctrl%PS_Initialize()
   call fdps_ctrl%VEO_Initialize('libvesph.so'//c_null_char)
   ...
   call fdps_ctrl%create_tree(tree_num_dens, &
                              ...
                              "calc_density_prepare_epj_for_force", "calc_density")
   ...
   call fdps_ctrl%create_tree(tree_num_hydro, &
                              ...
                              "calc_hydro_prepare_epj_for_force", "calc_hydro")
   ...
   offset_loop_duration = fdps_ctrl%get_wtime()
   file_output_time = 0.0d0
   ...
   do 
      ...
      if (mod(nstep,output_interval) == 0) then
         offset_file_output = fdps_ctrl%get_wtime()
         call output(fdps_ctrl,psys_num,nstep)
         call check_cnsrvd_vars(fdps_ctrl,psys_num)

         file_output_time = file_output_time +  fdps_ctrl%get_wtime() - offset_file_output
      end if
   ...
   !main_loop_duration = fdps_ctrl%get_wtime() - offset_loop_duration
   !call fdps_ctrl%dump_profile_tree(tree_num_dens, "Dens")
   !call fdps_ctrl%dump_profile_tree(tree_num_hydro, "Hydro")
   !write(*,*) 'Rank=',fdps_ctrl%get_rank(),' file_output_time=',file_output_time
   !write(*,*) 'Rank=',fdps_ctrl%get_rank(),' main_loop_duration=',main_loop_duration
   call fdps_ctrl%delete_tree(tree_num_dens)
   call fdps_ctrl%delete_tree(tree_num_hydro)
