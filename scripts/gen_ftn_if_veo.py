...
class Automatic_Generator:
    ...
    #for VEO
    def __mode_to_spj_cpp(self,mode_t):
        spj_t = "PS::SPJMonopole"
        if (mode_t.startswith("Quadrupole")):
            spj_t = "PS::SPJQuadrupole"
        if (mode_t.find("Symmetry") > 0):	
            return spj_t
        spj_i = 0
        for f in self.__multipole_kinds: 
            if (mode_t == f):  
                return self.__SPJs_cpp[spj_i]
            else:
                spj_i += 1
        return "undefinedSPJ"
    ...
    def __write_create_tree_cpp_impl(self,fh):
        ...
        for tree_t,force_t,epi_t,epj_t,mode_t in self.__tree_kinds:
            if (is_first):
                ...
                if_chars = "} else if"
            #spj_t = "ParticleSimulator::SPJMonopole"
            #if (mode_t.startswith("Quadrupole")):
            #    spj_t = "ParticleSimulator::SPJQuadrupole"
            spj_t = self.__mode_to_spj_cpp(mode_t)
            # (1) if & elif clauses
            if (tree_t == "Short"):
                text = """
            {IF_CHARS} (tree_info_ == "{TREE_T},{FORCE_T},{EPI_T},{EPJ_T},{MODE_T}") {{
               ...
               tree_data.pVeoFunctorEpEp = (void *)(new VeoFunctorEpEp<{EPI_T},{EPJ_T}>(pFuncPrepareEpjSymbol,pFuncEpEpSymbol));
               tree_data.pVeoFunctorEpSp = NULL;
                """.format(IF_CHARS=if_chars, \
                       TREE_T=tree_t, \
                       FORCE_T=force_t, \
                       EPI_T=epi_t, \
            ...
            {IF_CHARS} (tree_info_ == "{TREE_T},{FORCE_T},{EPI_T},{EPJ_T},{MODE_T}") {{
               ...
               tree_data.ptr  = (void *) tree;
               tree_data.info = tree_info_;
               VeoFunctorEpEp<{EPI_T},{EPJ_T}> *pfunc_ep_ep;
               pfunc_ep_ep = new VeoFunctorEpEp<{EPI_T},{EPJ_T}>(pFuncPrepareEpjSymbol,pFuncEpEpSymbol);
               tree_data.pVeoFunctorEpEp = (void *)pfunc_ep_ep;
               tree_data.pVeoFunctorEpSp = NULL;
               VeoFunctorEpSp<{EPI_T},{EPJ_T},{SPJ_T}> *pfunc_ep_sp;
	       pfunc_ep_sp= new VeoFunctorEpSp<{EPI_T},{EPJ_T},{SPJ_T}>(pFuncPrepareSpjSymbol,pFuncEpSpSymbol,*pfunc_ep_ep);
               tree_data.pVeoFunctorEpSp = (void *)pfunc_ep_sp;
               """.format(IF_CHARS=if_chars, \
                       TREE_T=tree_t, \
                       FORCE_T=force_t, \
                       EPI_T=epi_t, \
    ...
    def __write_delete_tree_cpp_impl(self,fh):
        # Set the indent
        indent = self.__get_cpp_indent(2)
        # Set the output text
        texts = []
        ...
        for tree_t,force_t,epi_t,epj_t,mode_t in self.__tree_kinds:
            ...
            # (1) if & elif clauses
            text = """
            {IF_CHARS} (p_tree_data->info == "{TREE_T},{FORCE_T},{EPI_T},{EPJ_T},{MODE_T}") {{
               VeoFunctorEpEp<{EPI_T},{EPJ_T}> *func_ep_ep_ptr = (VeoFunctorEpEp<{EPI_T},{EPJ_T}> *)p_tree_data->pVeoFunctorEpEp;
               //delete func_ep_ep_ptr;
               func_ep_ep_ptr->cleanup();
            """.format(IF_CHARS=if_chars, \
                       TREE_T=tree_t, \
                       FORCE_T=force_t, \
                       EPI_T=epi_t, \
        ...
        text = """
           ...
           if (myrank == 0) {
              std::string errmsg,func_name;
              errmsg = "cannot delete Tree `" + p_tree_data->info + "` ";
              func_name = "create_tree";
              print_errmsg(errmsg,func_name);
           }
           PS::Finalize();
    ...
    def __write_dump_profile_tree_cpp_impl(self,fh):
        # Set the indent
        indent = self.__get_cpp_indent(2)
        # Set the output text
        texts = []
        ...
        for tree_t,force_t,epi_t,epj_t,mode_t in self.__tree_kinds:
            ...
            {IF_CHARS} (p_tree_data->info == "{TREE_T},{FORCE_T},{EPI_T},{EPJ_T},{MODE_T}") {{
                   ...
                   tree->getTimeProfile().dump(p_title);
            """.format(IF_CHARS=if_chars, \
                       TREE_T=tree_t, \
                       FORCE_T=force_t, \
                       EPI_T=epi_t, \
        ...
        for fp_t,tree_t,force_t,epi_t,epj_t,mode_t in self.__calc_force_kinds:
            ...
            if (tree_t == "Short"):
                ...
                {IF_CHARS} (info == "{FP_T},{TREE_T},{FORCE_T},{EPI_T},{EPJ_T},{MODE_T}") {{
                   ...
                   psys = (PS::ParticleSystem<{FP_T}> *) psys_ptr;
                   VeoFunctorEpEp<{EPI_T},{EPJ_T}> *pfunc_ep_ep;
                   pfunc_ep_ep = (VeoFunctorEpEp<{EPI_T},{EPJ_T}> *)func_ep_ep_ptr;
                   // Perform API
                ...
                {IF_CHARS} (info == "{FP_T},{TREE_T},{FORCE_T},{EPI_T},{EPJ_T},{MODE_T}") {{
                   ...
                   pfunc_ep_ep = (VeoFunctorEpEp<{EPI_T},{EPJ_T}> *)func_ep_ep_ptr;
                   VeoFunctorEpSp<{EPI_T},{EPJ_T},{SPJ_T}> *pfunc_ep_sp;
                   pfunc_ep_sp = (VeoFunctorEpSp<{EPI_T},{EPJ_T},{SPJ_T}> *)func_ep_sp_ptr;
                   // Perform API
                   if (list_mode == MAKE_LIST) {{
        ...
        for tree_t,force_t,epi_t,epj_t,mode_t in self.__tree_kinds:
            ...
            if (tree_t == "Short"):
                ...
                {IF_CHARS} (info == "{TREE_T},{FORCE_T},{EPI_T},{EPJ_T},{MODE_T}") {{
                   ...
                   PS::TreeForForce{TREE_T}<{FORCE_T},{EPI_T},{EPJ_T}>::{MODE_T} *tree;
                   tree = (PS::TreeForForce{TREE_T}<{FORCE_T},{EPI_T},{EPJ_T}>::{MODE_T} *) tree_ptr;
                   VeoFunctorEpEp<{EPI_T},{EPJ_T}> *pfunc;
                   pfunc = (VeoFunctorEpEp<{EPI_T},{EPJ_T}> *)func_ep_ep_ptr;
                   // Perform API
                   //tree->calcForceMakingTree(pfunc_ep_ep, *dinfo, clear);
                   tree->calcForceMakingTree(*pfunc, *dinfo, clear);
                """.format(IF_CHARS=if_chars, \
                           TREE_T=tree_t, \
                           FORCE_T=force_t, \
                           EPI_T=epi_t, \
        ...
        for fp_t,tree_t,force_t,epi_t,epj_t,mode_t in self.__calc_force_kinds:
            ...
            if (tree_t == "Short"):
                ...
                {IF_CHARS} (info == "{FP_T},{TREE_T},{FORCE_T},{EPI_T},{EPJ_T},{MODE_T}") {{
                   ...
                   tree->calcForceAndWriteBack(*pfunc, *psys, clear);
                """.format(IF_CHARS=if_chars, \
                           FP_T=fp_t, \
                           TREE_T=tree_t, \
                           FORCE_T=force_t, \
    ...
    def generate(self,output_dir):
        ...
        blueprint_dir = os.path.abspath(os.path.dirname(__file__)) \
                      + "/../src/fortran_interface/blueprints_veo"
        ...
        pat_delete_tree          = re.compile(r"fdps-autogen:delete_tree;")
        pat_dump_profile_tree    = re.compile(r"fdps-autogen:dump_profile_tree;")
        ...
        for line in ifh:
            ...
            if (pat_delete_tree.search(line)):
                self.__write_delete_tree_cpp_impl(ofh)
            if (pat_dump_profile_tree.search(line)):
                self.__write_dump_profile_tree_cpp_impl(ofh)
