...
#include <ftrace.h>
#else
#define ftrace_region_begin(a)
#define ftrace_region_end(a)
#endif
...
#if defined(_SXDEBUG)
#define ASE_DEBUG_PRINT(rank,a,b) { std::cout<<"rank="<<(rank)<<" function: "<<__FUNCTION__<<" "<<(a)<<"="<<(b)<<std::endl; }
#if defined(PARTICLE_SIMULATOR_MPI_PARALLEL)
#define PS_ASE_DEBUG_PRINT(a,b) { std::cout<<"rank="<<Comm::getRank()<<" function: "<<__FUNCTION__<<" "<<(a)<<"="<<(b)<<std::endl; }
#define PS_ASE_DEBUG_PRINT2(a,b,c,d) { std::cout<<"rank="<<Comm::getRank()<<" function: "<<__FUNCTION__<<" "<<(a)<<"="<<(b)<<" "<<(c)<<"="<<(d)<<std::endl; }
#else
#define PS_ASE_DEBUG_PRINT(a,b) { std::cout<<" function: "<<__FUNCTION__<<" "<<(a)<<"="<<(b)<<std::endl; }
#define PS_ASE_DEBUG_PRINT2(a,b,c,d) { std::cout<<" function: "<<__FUNCTION__<<" "<<(a)<<"="<<(b)<<" "<<(c)<<"="<<(d)<<std::endl; }
#endif
#else
#define ASE_DEBUG_PRINT(rank,a,b)
#define PS_ASE_DEBUG_PRINT(a,b)
#define PS_ASE_DEBUG_PRINT2(a,b,c,d)
#endif
...
#if defined(_SXDEBUG)
        // スレッド毎のツリー構築時間
        F64 calc_force__core__walk_tree[N_MAX_THREADS];
        F64 calc_force__core__multiwalk_tree;
#else
...
#if defined(_SXDEBUG)
        F64 calc_force__core__transfer;
        // スレッド毎のユーザ定義関数時間
        F64 calc_force__core__dispatch[N_MAX_THREADS];
        F64 calc_force__core__multi_dispatch;
#else
	F64 calc_force__core__dispatch;
...
#if defined(_SXDEBUG)
        F64 calc_force__core__make_list_usingtree;
        S64 dispatch_ep_ep_count[N_MAX_THREADS];
        S64 dispatch_ep_sp_count[N_MAX_THREADS];
        F64 dispatch_ep_ep_time[N_MAX_THREADS];
        F64 dispatch_ep_sp_time[N_MAX_THREADS];
        S64 total_ep_ep_epi[N_MAX_THREADS];
        S64 total_ep_ep_epj[N_MAX_THREADS];
        S64 total_ep_sp_epi[N_MAX_THREADS];
        S64 total_ep_sp_spj[N_MAX_THREADS];

        void addEpEpDispatchInfo(int ith, F64 call_time, S32 n_epi, S32 n_epj) {
                dispatch_ep_ep_count[ith] += 1;
                dispatch_ep_ep_time[ith] += call_time;
                total_ep_ep_epi[ith] += (S64)n_epi;
                total_ep_ep_epj[ith] += (S64)n_epj;
        }

        void addEpSpDispatchInfo(int ith, F64 call_time, S32 n_epi, S32 n_spj) {
                dispatch_ep_sp_count[ith] += 1;
                dispatch_ep_sp_time[ith] += call_time;
                total_ep_sp_epi[ith] += (S64)n_epi;
                total_ep_sp_spj[ith] += (S64)n_spj;
        }
#endif
...
#if defined(_SXDEBUG)
        void dumpValue(const char * pTreeName, const char * pValueName, F64 value, int index = -1) const {
            if (index >= 0) {
                fprintf(stdout, "Rank=%d %s %s[%d]=%lf\n", Comm::getRank(), pTreeName, pValueName, index, value);
            } else {
                fprintf(stdout, "Rank=%d %s %s=%lf\n", Comm::getRank(), pTreeName, pValueName, value);
            }
        }

        void dumpValue(const char * pTreeName, const char * pValueName, S64 value, int index = -1) const {
            if (index >= 0) {
                fprintf(stdout, "Rank=%d %s %s[%d]=%ld\n", Comm::getRank(), pTreeName, pValueName, index, value);
            } else {
                fprintf(stdout, "Rank=%d %s %s=%ld\n", Comm::getRank(), pTreeName, pValueName, value);
            }
        }

        void dump(const char* pTreeName = "default") const {
            dumpValue(pTreeName, "collect_sample_particle", collect_sample_particle);
            dumpValue(pTreeName, "decompose_domain", decompose_domain);
            dumpValue(pTreeName, "exchange_particle", exchange_particle);
            dumpValue(pTreeName, "set_particle_local_tree", set_particle_local_tree);
            dumpValue(pTreeName, "set_particle_global_tree", set_particle_global_tree);
            dumpValue(pTreeName, "make_local_tree", make_local_tree);
            dumpValue(pTreeName, "make_global_tree", make_global_tree);
            dumpValue(pTreeName, "set_root_cell", set_root_cell);
            dumpValue(pTreeName, "morton_sort_local_tree", morton_sort_local_tree);
            dumpValue(pTreeName, "morton_sort_global_tree", morton_sort_global_tree);
            dumpValue(pTreeName, "link_cell_local_tree", link_cell_local_tree);
            dumpValue(pTreeName, "link_cell_global_tree", link_cell_global_tree);
            dumpValue(pTreeName, "calc_moment_local_tree", calc_moment_local_tree);
            dumpValue(pTreeName, "calc_moment_global_tree", calc_moment_global_tree);
            dumpValue(pTreeName, "exchange_LET_tot",exchange_LET_tot);
            dumpValue(pTreeName, "add_moment_as_sp_global",add_moment_as_sp_global);
            dumpValue(pTreeName, "calc_force", calc_force) ;
            dumpValue(pTreeName, "calc_force__core__transfer", calc_force__core__transfer);
            for(int i=0; i<Comm::getNumberOfThread(); i++) {
                dumpValue(pTreeName, "calc_force__core__walk_tree", calc_force__core__walk_tree[i], i);
                dumpValue(pTreeName, "calc_force__core__dispatch", calc_force__core__dispatch[i], i);
                if (dispatch_ep_ep_count[i] > 0) {
                        dumpValue(pTreeName, "calc_force__core__dispatch_ep_ep_time", dispatch_ep_ep_time[i], i);
                        dumpValue(pTreeName, "calc_force__core__dispatch_ep_ep_count", dispatch_ep_ep_count[i], i);
                        dumpValue(pTreeName, "calc_force__core__dispatch_ep_ep_ave_epi", total_ep_ep_epi[i]/dispatch_ep_ep_count[i], i);
                        dumpValue(pTreeName, "calc_force__core__dispatch_ep_ep_ave_epj", total_ep_ep_epj[i]/dispatch_ep_ep_count[i], i);
                }
                if (dispatch_ep_sp_count[i] > 0) {
                        dumpValue(pTreeName, "calc_force__core__dispatch_ep_sp_time", dispatch_ep_sp_time[i], i);
                        dumpValue(pTreeName, "calc_force__core__dispatch_ep_sp_count", dispatch_ep_sp_count[i], i);
                        dumpValue(pTreeName, "calc_force__core__dispatch_ep_sp_ave_epi", total_ep_sp_epi[i]/dispatch_ep_sp_count[i], i);
                        dumpValue(pTreeName, "calc_force__core__dispatch_ep_sp_ave_spj", total_ep_sp_spj[i]/dispatch_ep_sp_count[i], i);
                }
            }
            dumpValue(pTreeName, "calc_force__core__multiwalk_tree", calc_force__core__multiwalk_tree);
            dumpValue(pTreeName, "calc_force__core__multi_dispatch", calc_force__core__multi_dispatch);
            dumpValue(pTreeName, "calc_force__make_ipgroup", calc_force__make_ipgroup);
            dumpValue(pTreeName, "calc_moment_local_tree",calc_moment_local_tree);
            dumpValue(pTreeName, "calc_moment_global_tree",calc_moment_global_tree);
            dumpValue(pTreeName, "make_LET_1st",make_LET_1st);
            dumpValue(pTreeName, "make_LET_2nd",make_LET_2nd);
            dumpValue(pTreeName, "exchange_LET_1st",exchange_LET_1st);
            dumpValue(pTreeName, "exchange_LET_2nd",exchange_LET_2nd);
            dumpValue(pTreeName, "write_back",write_back);
            dumpValue(pTreeName, "calc_force__core__make_list_usingtree", calc_force__core__make_list_usingtree);
        }
#else
	    ...
	    decompose_domain__barrier=decompose_domain__postprocess=0;
            ...
            for(int i=0; i<N_MAX_THREADS; i++) {
                calc_force__core__walk_tree[i] = 0.0;
                calc_force__core__dispatch[i] = 0.0;

                dispatch_ep_ep_count[i] = 0;
                dispatch_ep_sp_count[i] = 0;
                dispatch_ep_ep_time[i] = 0.0;
                dispatch_ep_sp_time[i] = 0.0;
                total_ep_ep_epi[i] = 0;
                total_ep_ep_epj[i] = 0;
                total_ep_sp_epi[i] = 0;
                total_ep_sp_spj[i] = 0;
            }
            calc_force__core__multiwalk_tree=0.0;
            calc_force__core__multi_dispatch=0.0;
#endif
        }
...
#if defined(_SXDEBUG)
            for(int i=0; i<N_MAX_THREADS; i++) {
                ret.calc_force__core__walk_tree[i] = this->calc_force__core__walk_tree[i] + rhs.calc_force__core__walk_tree[i];
                ret.calc_force__core__dispatch[i] = this->calc_force__core__dispatch[i] + rhs.calc_force__core__dispatch[i];
            }
            ret.calc_force__core__multiwalk_tree= this->calc_force__core__multiwalk_tree + rhs.calc_force__core__multiwalk_tree;
            ret.calc_force__core__multi_dispatch= this->calc_force__core__multi_dispatch + rhs.calc_force__core__multi_dispatch;
            ret.calc_force__core__make_list_usingtree = this->calc_force__core__make_list_usingtree + rhs.calc_force__core__make_list_usingtree;
            ret.calc_force__core__keep_list = this->calc_force__core__keep_list + rhs.calc_force__core__keep_list;
            ret.calc_force__core__transfer = this->calc_force__core__transfer + rhs.calc_force__core__transfer;
#else
	    ...
	    morton_key_local_tree = 0;
            ...
            calc_force__core__multiwalk_tree=0.0;
            calc_force__core__multi_dispatch=0.0;
            calc_force__core__make_list_usingtree = 0.0;
